...
PndLmdFitFacade::calcRange(const PndLmdAbstractData &lmd_abs_data,
  ...
  if (lmd_abs_data.getSecondaryDimension().is_active) {
    ...
    if (est_options.getFitRangeY().is_active) {
      ...
      std::cout << "fit range was active and changed the range to "
                << temp_range.range_low << "-" << temp_range.range_high
                << std::endl;
    }
...
double PndLmdFitFacade::calcHistIntegral(
    const TH1D *hist, std::vector<DataStructs::DimensionRange> range) const {
  ...
  for (int i = 0; i < hist->GetNbinsX(); i++) {
    ...
    if (hist->GetBinCenter(i) - hist->GetBinWidth(i) / 2 <
            range[0].range_high &&
...
double PndLmdFitFacade::calcHistIntegral(
    const TH2D *hist, std::vector<DataStructs::DimensionRange> range) const {
  // just determine the bin range over which we have to calculate
  if (range.size() < 2)
    return 0.0;
  ...
  for (int i = 0; i < hist->GetNbinsX(); i++) {
    if (hist->GetXaxis()->GetBinCenter(i) -
  ...
  for (int i = 0; i < hist->GetNbinsY(); i++) {
    if (hist->GetYaxis()->GetBinCenter(i) -
                hist->GetYaxis()->GetBinWidth(i) / 2 <
            range[1].range_low &&
...
std::shared_ptr<Data>
PndLmdFitFacade::createData1D(const PndLmdHistogramData &lmd_hist_data) const {
  std::shared_ptr<Data> data(new Data(1));
  data_helper.fillBinnedData(data, lmd_hist_data.get1DHistogram());
  return data;
}
std::shared_ptr<Data>
PndLmdFitFacade::createData2D(const PndLmdHistogramData &lmd_hist_data) const {
...
EstimatorOptions
PndLmdFitFacade::constructEstimatorOptionsFromConfig(const ptree &pt) const {
...
PndLmdFitFacade::constructFreeFitParameterListFromConfig(
...
void PndLmdFitFacade::freeParametersForModel(
...
void PndLmdFitFacade::initBeamParametersForModel(
  ...
  if (model_opt_ptree.get<bool>("divergence_smearing_active")) {
    ...
    // bool optional
    boost::optional<double> v1 =
        model_opt_ptree.get_optional<double>("beam_div_x");
    boost::optional<double> v2 =
        model_opt_ptree.get_optional<double>("beam_div_y");
    ...
    current_model->getModelParameterSet()
        ...
        ->setValue(start_div_x);
    current_model->getModelParameterSet()
        .getModelParameter("gauss_sigma_var2")
        ->setValue(start_div_y);
...
PndLmdFitOptions
PndLmdFitFacade::createFitOptions(const PndLmdAbstractData &lmd_data) const {
  ...
  ptree model_option_ptree =
      fit_config_ptree.get_child("fit.fit_model_options");
...
PndLmdFitDataBundle PndLmdFitFacade::doLuminosityFits(
  ...
  for (auto &lmd_data : lmd_data_vec) {
    cout << "Data dimensions: "
         << lmd_data.getPrimaryDimension().createDimensionLabel();
    if (lmd_data.getSecondaryDimension().is_active)
      cout << " vs. "
           << lmd_data.getSecondaryDimension().createDimensionLabel();
    cout << "\n";
    ...
    if (fit_options.getModelOptionsPropertyTree().get<bool>(
      ...
      if (resolution_map_pool.size() > 0) {
        if (resolution_map_pool.begin()->getHitMap().size() == 0) {
          std::cout << "Requesting fit with resolution smearing, however "
                       "resolution map data is empty!"
                    << "Hence skipping this fit!\n";
          continue;
        }
        model_factory.setResolutionMapData(*resolution_map_pool.begin());
      } else {
        std::cout << "Requesting fit with resolution smearing, however no "
                     "resolution map data was specified!"
                  << "Hence skipping this fit!\n";
        continue;
      }
    }
    ...
    if (fit_options.getModelOptionsPropertyTree().get<bool>(
      ...
      matching_acc =
          lmd_data_facade.getMatchingAcceptances(acceptance_pool, lmd_data);
...
void PndLmdFitFacade::fitElasticPPbar(PndLmdAngularData &lmd_data) {
  ...
  if (fit_options.model_opt_map["divergence_smearing_active"] == "true") {
    ...
    std::vector<DataStructs::DimensionRange> range =
        calcRange(lmd_data, fit_options.getEstimatorOptions());
    ...
    for (auto const &var : var_names) {
      for (auto const param : no_div_fit_res_params) {
        if (param.name.second == var) {
          model->getModelParameterSet().getModelParameter(var)->setValue(
              param.value);
    ...
    if (has_divergence_parameters) {
      std::vector<mydouble> start_values =
          model_fit_facade.findGoodStartParameters(scan_var_names, {1.5, 2.0});
...
PndLmdFitFacade::generateModel(const PndLmdAngularData &lmd_data) {
  PndLmdFitOptions fit_options(createFitOptions(lmd_data));
...
void PndLmdFitFacade::scanEstimatorSpace(
    PndLmdHistogramData &lmd_hist_data, const PndLmdFitOptions &fit_options,
  ...
  // TH2D hist2d("name", "div likelihood scan", 60, 0.0000997, 0.000103, 60,
  // 0.000194, 0.000206);
...
void PndLmdFitFacade::fitVertexData(
  ...
  for (unsigned int i = 0; i < lmd_data.size(); i++) {
    ...
    if (hist->Integral() < 400) {
      std::cout << "WARNING: not performing fit due to low statistics of the "
                   "vertex distribution (below 400 events)\n";
      continue;
    }
    ...
    if (!user_range.is_active) {
      ...
      if (lmd_data[i].getPrimaryDimension().dimension_options.track_type ==
        ...
        fit_range.is_active = true;
                     ...
                     .getPrimaryDimension()
                     .dimension_options.track_type == LumiFit::RECO) {
        fit_range.range_low = hist_mean - sigma_range * hist_rms;
        fit_range.range_high = hist_mean + sigma_range * hist_rms;
      ...
      if (fit_range.is_active) {
        if (fit_range.range_low < hist->GetXaxis()->GetXmin()) {
          ...
          std::cout << "WARNING: Data is underflowing the histogram! Something "
                       "in track reco went wrong"
                    << ", you should check this. Proceeding, but be aware that "
        ...
        if (fit_range.range_high > hist->GetXaxis()->GetXmax()) {
          ...
          std::cout << "WARNING: Data is overflowing the histogram! Something "
                       "in track reco went wrong"
                    << ", you should check this. Proceeding, but be aware that "
                       "this can lead to incorrect results! \n";
        }
    ...
    std::shared_ptr<Model1D> vertex_model =
        model_factory.generate1DVertexModel(model_opt_ptree);
