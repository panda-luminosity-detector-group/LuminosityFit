...
private:
  ...
  generatePlotRange(const PndLmdAbstractData &lmd_abs_data,
  ...
  create2DVisualizationProperties(const EstimatorOptions &est_options,
                                  const PndLmdAbstractData &lmd_abs_data) const;

  ModelVisualizationProperties1D
  create1DVisualizationProperties(const EstimatorOptions &est_options,
                                  const PndLmdAbstractData &lmd_abs_data) const;
#endif /* __CINT __ */

  bool
  compareLumiModelOptions(const boost::property_tree::ptree &pt,
...
public:
  ...
  TH2D *create2DHistogramFromFitResult(
      const PndLmdFitOptions &fit_opt,
      const PndLmdElasticDataBundle &elastic_data_bundle);

  TGraphAsymmErrors *
  createVertexGraphFromFitResult(const PndLmdFitOptions &fit_opt,
  ...
  TGraphAsymmErrors *
  createSmearingGraphFromFitResult(const PndLmdFitOptions &fit_opt,
                                   const PndLmdHistogramData &res_data) const;
  ...
  TGraphAsymmErrors *
  generateDPMModelPartGraph(double plab,
  ...
  makeAcceptanceBundle1D(const PndLmdAcceptance &acc) const;

  NeatPlotting::PlotBundle
  makeAcceptanceBundle2D(const PndLmdAcceptance &acc) const;
  ...
  NeatPlotting::PlotBundle
  makeResidualPlotBundle1D(PndLmdAngularData &data,
                           const PndLmdFitOptions &fit_options);

  TH2D *makeResolutionHistogram(const PndLmdMapData &res) const;

  /*NeatPlotting::PlotBundle2D makeResolutionGraphBundle2D(
   PndLmdHistogramData& res);*/

  NeatPlotting::PlotBundle
  makeVertexGraphBundle1D(const PndLmdHistogramData &data) const;
  ...
  /*
   ...
   std::map<PndLmdLumiFitOptions, NeatPlotting::PlotBundle1D>
   makeGraphBundles1D( std::vector<PndLmdAngularData> &data_vec,
  ...
  clusterIntoGroups(const std::vector<PndLmdAngularData> &data_vec) const;
  ...
  createLowerFitRangeDependencyGraphBundle(
      const std::vector<PndLmdAngularData> &prefiltered_data,
      const LmdDimensionOptions &dim_opt) const;
  ...
  NeatPlotting::PlotBundle
  createAcceptanceErrorPlot(std::vector<PndLmdAcceptance> &accs) const;

  void createAcceptanceComparisonBooky(
      std::vector<std::pair<PndLmdAcceptance, PndLmdAcceptance>> &acc_matches);

  NeatPlotting::Booky
  makeVertexFitResultBooky(const std::vector<PndLmdHistogramData> &data_vec);

  void
  makeVertexDifferencesBooky(std::vector<PndLmdHistogramData> &res_vec,
  ...
  NeatPlotting::Booky
  create2DFitResultPlots(const PndLmdFitDataBundle &fit_data_bundle);
  ...
  /*	 std::map<LumiFit::PndLmdFitModelOptions,
   ...
   void makeResolutionDifferencesBooky(std::vector<PndLmdHistogramData>
   &res_vec, std::vector<PndLmdHistogramData> &res_vec_ref);
   ...
   std::map<LumiFit::LmdSimIPParameters,
   std::map<LumiFit::LmdDimensionType, std::vector<PndLmdVertexData> > > >
   &ip_data_clusters);
   */
};
} // namespace LumiFit
