...
enum LmdEstimatorType { CHI2, LOG_LIKELIHOOD };

#ifndef __CINT__
const std::unordered_map<std::string, LmdEstimatorType>
    StringToLmdEstimatorType = {{"CHI2", CHI2},
                                {"LOG_LIKELIHOOD", LOG_LIKELIHOOD}};

const std::unordered_map<LmdEstimatorType, std::string>
    LmdEstimatorTypeToString = {{CHI2, "CHI2"},
                                {LOG_LIKELIHOOD, "LOG_LIKELIHOOD"}};
#endif

enum LmdDataType { HISTOGRAM, EFFICIENCY };
...
enum LmdTrackType { MC, MC_ACC, RECO, DIFF_RECO_MC };

enum LmdTrackParamType { IP, LMD };

#ifndef __CINT__
const std::unordered_map<std::string, LmdTrackParamType>
    StringToTrackParamType = {{"IP", IP}, {"LMD", LMD}};
#endif

enum ModelType { GAUSSIAN, DOUBLE_GAUSSIAN, ASYMMETRIC_GAUSSIAN, UNIFORM };
...
#ifndef __CINT__
const std::unordered_map<std::string, ModelStructs::InterpolationType>
    StringToInterpolationType = {{"CONSTANT", ModelStructs::CONSTANT},
                                 {"LINEAR", ModelStructs::LINEAR},
                                 {"SPLINE", ModelStructs::SPLINE}};
#endif

enum TransformationOption { APPROX, CORRECT };

#ifndef __CINT__
const std::unordered_map<std::string, TransformationOption>
    StringToTransformationOption = {{"APPROX", APPROX}, {"CORRECT", CORRECT}};
const std::unordered_map<TransformationOption, std::string>
    TransformationOptionToString = {{APPROX, "APPROX"}, {CORRECT, "CORRECT"}};
#endif
...
const std::unordered_map<std::string, DPMElasticParts> StringToDPMElasticParts =
    {{"COUL", COUL},
     {"INT", INT},
     {"HAD", HAD},
     {"HAD_RHO_B_SIGTOT", HAD_RHO_B_SIGTOT},
     {"ALL_RHO_B_SIGTOT", ALL_RHO_B_SIGTOT},
     {"ALL", ALL}};
...
struct LmdDimensionOptions : public TObject {
  ...
  bool operator>(const LmdDimensionOptions &rhs) const { return (rhs < *this); }
...
public:
  LmdDimensionRange() : range_low(0.0), range_high(0.0) {}

  double getDimensionLength() const { return (getRangeHigh() - getRangeLow()); }

  double getDimensionMean() const {
    return (getRangeHigh() + getRangeLow()) / 2.0;
  }

  double getRangeLow() const { return range_low; }

  double getRangeHigh() const { return range_high; }

  void setRangeLow(double range_low_) { range_low = range_low_; }

  void setRangeHigh(double range_high_) { range_high = range_high_; }
...
struct BinDimension {
  ...
  bool operator>(const BinDimension &rhs) const { return (rhs < *this); }
};
...
inline double calculateBinOverlap(LumiFit::BinDimension &reco_bin,
  ...
  double reso_bin_area =
      (reso_bin.x_range.range_high - reso_bin.x_range.range_low) *
  ...
  return (overlap.x_range.range_high - overlap.x_range.range_low) *
         (overlap.y_range.range_high - overlap.y_range.range_low) /
         reso_bin_area;
}
#endif
...
struct LmdDimension : public TObject {
   ...
   * axis as labels. Generated from the #LmdDimensionType and
   * #LmdTrackParamType.
   */
...
"--------------------------------------------------"
        ...
        std::string getLabel() const {
                ...
                ss << "IP_pos_" << offset_x_mean << "_" << offset_x_width << "_"
                                << offset_y_mean << "_" << offset_y_width << "_"
<< offset_z_mean << "_"
                                << offset_z_width << "-beam_tilt_" <<
tilt_x_mean << "_" << tilt_x_width
        ...
        double getDependencyValue(const LmdSimIPParameters &param_template)
const { if (1.0 == param_template.offset_x_mean) return offset_x_mean; if (1.0
== param_template.offset_x_width) return offset_x_width; if (1.0 ==
param_template.offset_y_mean) return offset_y_mean; if (1.0 ==
param_template.offset_y_width) return offset_y_width; if (1.0 ==
param_template.offset_z_mean) return offset_z_mean; if (1.0 ==
param_template.offset_z_width) return offset_z_width;
