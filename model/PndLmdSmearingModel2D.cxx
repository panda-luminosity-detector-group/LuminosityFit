...
const std::vector<ContributorCoordinateWeight>& PndLmdSmearingModel2D::findNearestNeighbour(
  ...
  for (unsigned int i = 0; i < last_found_neighbour_indices.size(); ++i) {
    unsigned int index(last_found_neighbour_indices[i] + 1);
    if (smearing_parameterization.size() > index) {
      const RecoBinSmearingContributions &temprecobin(
          smearing_parameterization[index]);
...
void PndLmdSmearingModel2D::setSearchDistances(double search_distance_x_,
    double search_distance_y_) {
