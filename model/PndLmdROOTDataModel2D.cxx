...
PndLmdROOTDataModel2D::PndLmdROOTDataModel2D(std::string name_) :
		Model2D(name_), using_acceptance_bounds(false), eff(0), acc_x_range_low(
				0.0), acc_x_range_high(0.0), acc_y_range_low(0.0), acc_y_range_high(0.0) {
}
...
std::pair<LumiFit::LmdDimensionRange, LumiFit::LmdDimensionRange> PndLmdROOTDataModel2D::getDataDimension() const {
...
TEfficiency *PndLmdROOTDataModel2D::getEfficieny() const {
	return eff;
}
...
TGraph2DErrors *PndLmdROOTDataModel2D::getGraph() const {
	return graph;
}
...
std::pair<std::pair<double, double>, std::pair<double, double> > PndLmdROOTDataModel2D::getAcceptanceBounds() const {
...
std::pair<double, double> PndLmdROOTDataModel2D::getUncertaincy(
