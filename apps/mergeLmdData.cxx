...
bootstrapData(vector<string> found_files,
...
template <class T>
void mergeData(const vector<string> &found_files, const string &outfile_path,
...
template <class T>
void mergeData(vector<string> found_files, TFile *output_file) {
...
void displayInfo() {
  ...
  std::cout << std::endl;
  std::cout << "Note: The type value is specified as a string, in which the 4 "
               "letters\n"
...
int main(int argc, char *argv[]) {
  ...
  if (is_data_path_set && is_type_set) {
    ...
    if (dir_pattern.compare("") != 0) {
      // ------ get files ------
      found_files =
          lmd_data_facade.findFile(data_path, dir_pattern, filename_pattern);
    } else {
    ...
    size_t max_bootstrap_samples =
        binominalCoefficient(found_files.size(), samples.second);
