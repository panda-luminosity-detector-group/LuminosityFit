...
struct Stats {
  ...
  Stats()
      : mean_relative_precision(0.0), mean_entries(0),
        mean_num_contributors(0) {}
};
...
Stats calcStats(const PndLmdMapData &res_map) {
  ...
  Stats st;
  st.relative_error_distribution =
      new TH2D("asdf", "", res_map.getPrimaryDimension().bins, -0.013, 0.013,
  ...
  st.entries_distribution_2d =
      new TH2D("asdf", "", res_map.getPrimaryDimension().bins, -0.013, 0.013,
               res_map.getPrimaryDimension().bins, -0.013, 0.013);
  st.total_distribution_2d =
      new TH2D("asdf", "", res_map.getPrimaryDimension().bins, -0.013, 0.013,
               res_map.getPrimaryDimension().bins, -0.013, 0.013);
...
void analyzeResolutionMaps(std::vector<std::string> &res_map_files) {
  ...
  for (auto const &res_map_file : res_map_files) {
    ...
    std::vector<PndLmdMapData> datavec =
        lmd_data_facade.getDataFromFile<PndLmdMapData>(f);
    ...
    /* LumiFit::LmdDimensionOptions lmd_dim_opt;
     ...
     filter(lmd_dim_opt); datavec =
     lmd_data_facade.filterData<PndLmdAngularData>( datavec, filter);*/
