...
void checkFramework(unsigned int num_events) {
	...
	for (int sample = 0; sample < samples; sample++) {
		...
		model1d->getModelParameterSet().getModelParameter("gauss_sigma")->setParameterFixed(
				true);
		model1d->getModelParameterSet().getModelParameter("gauss_mean")->setValue(
				mean);
		model1d->getModelParameterSet().getModelParameter("gauss_mean")->setParameterFixed(
				true);
		model1d->getModelParameterSet().getModelParameter("gauss_amplitude")->setValue(
				refamp);
	...
	for (std::map<double, TH1D*>::iterator histogram_iter =
			root_histograms.begin(); histogram_iter != root_histograms.end();
			histogram_iter++) {
		histogram_iter->second->Write();
	}
