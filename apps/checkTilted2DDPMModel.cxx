...
int main(int argc, char* argv[]) {
	...
	shared_ptr<Model> model_old = model_factory.generateModel(model_opt,
			data);
	if (model_old->init()) {
	...
	shared_ptr<Model> model_fast = model_factory.generateModel(model_opt,
			data);
	if (model_fast->init()) {
		std::cout << "Error: not all parameters have been set!" << std::endl;
	}
	...
	integral_range_thetaphi[1].range_low = atan2(
			integral_range_thetaxy[1].range_low, integral_range_thetaxy[0].range_low);
	integral_range_thetaphi[1].range_high = atan2(
			integral_range_thetaxy[1].range_high,
