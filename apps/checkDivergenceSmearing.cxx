...
void plotSmeared2DModel(unsigned int nthreads, unsigned int scale_,
  ...
  /* const double var_min = -10.0;
   ...
   vis_prop_pair); TH2D* signal_hist =
   root_plotter.createHistogramFromModel2D(signal_model, vis_prop_pair); TH2D*
   div_hist = root_plotter.createHistogramFromModel2D(divergence_model,
   vis_prop_pair);
  ...
  /*lmd_runtime_config.readFitConfigFile(config_file_url);
   ...
   lmd_fit_facade.createModel2D(my_lmd_data_vec[0]);
