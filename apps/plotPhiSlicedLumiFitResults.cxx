 ...
 * General information about the individual classes of the LmdFit framework can
 * be found in the doxygen manual Run it like this:
 *
 * ./makeLumiFitPlots dir_to_elastic_scattering_data
 *
 * note: the dir_to_elastic_scattering_data should contain the fit_result.root
 * root file that contains the data and fit result on which fit were
 * performed...
 *
 * The usage of this macro is straight forward and uses primarily the
 * PndLmdResultPlotter class (which is a helper class for creating plots on
 * luminosity fits). -First you read in a PndLmdAngularData object that was
 * saved by the runLumi6Fit.C macro -Then you create so graph bundles that
 * contain all the information of the fit results and data in form of ROOT
 * objects. -Finally you can pass these bundles to functions of the plotter and
 * generate different type of plots
 */
...
void plotMultipleLumiFitResults(std::vector<std::string> paths,
  ...
  for (unsigned int i = 0; i < paths.size(); i++) {
    // ------ get files -------------------------------------------------------
    TFile *fdata =
        new TFile((paths[i] + "/lmd_fitted_data.root").c_str(), "READ");

    const boost::regex my_filename_filter(
        "phi_slice_(-?\\d*)", boost::regex::extended | boost::regex::icase);
    ...
    // read in data from a root file which will return a map of pointers to
    ...
    if (fit_results.begin()->second->getModelFitResult().getFitStatus() == 0) {
      ...
      std::cout << (lumi - lumi_ref) / lumi_ref << " +- " << lumi_err / lumi_ref
                << std::endl;
