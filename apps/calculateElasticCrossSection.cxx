...
void calculateElasticCrossSection(double momentum, double lower_bound,
  ...
  std::shared_ptr<Model> approx_model =
      model_factory.generateModel(approx_model_opt, data);
  ...
  std::cout << "Integrated total elastic cross section in theta range ["
            << lower_bound << " - " << upper_bound << "] is " << integral
            << " mb" << std::endl;

  // ok here we calculated the difference between the two transformations
  // (approx - correct)
  ...
  std::cout << time_span_approx.count() << " " << time_span_correct.count()
            << std::endl;
  g->Draw("AC");
