...
// create and fill data map first of all
	...
	// ------ get files -------------------------------------------------------
	std::vector<std::string> file_paths = lmd_data_facade.findFilesByName(path,
			"merge_data", "lmd_fitted_data.root");
		...
		if (full_phi_vec.size() > 0) {
			...
			for (unsigned int reco_data_obj_index = 0;
				...
				for (fit_result = fit_results.begin(); fit_result != fit_results.end();
					...
					if (fit_result->first.getFitModelOptions().resolution_smearing_active) {
						...
						NeatPlotting::PlotBundle data_only_reco_plot_bundle = lmd_plotter.makeGraphBundle(
								full_phi_reco_data_vec[reco_data_obj_index], temp, false);
						data_only_reco_plot_bundle.plot_axis.x_axis_range.low = plot_range_low;
						data_only_reco_plot_bundle.plot_axis.x_axis_range.high = plot_range_high;
						...
						c.SaveAs(filepath.str().c_str());
					} else if (fit_result->first.getFitModelOptions().acceptance_correction_active
							&& !fit_result->first.getFitModelOptions().resolution_smearing_active) {
					...
					std::pair<double, double> lumi =
							lmd_plotter.calulateRelDiff(
									lmd_plotter.getLuminosity(fit_result->second),
									lmd_plotter.getLuminosityError(fit_result->second),
									full_phi_reco_data_vec[reco_data_obj_index].getReferenceLuminosity());
			...
			if (full_phi_mc_data_vec.size() == 1) {
				...
				if (fit_results.size() > 0) {
					...
					NeatPlotting::PlotBundle mc_plot_bundle =
							lmd_plotter.makeGraphBundle(full_phi_mc_data_vec[0],
									fit_results.begin()->first);
