...
double gaus2d(double *x, double *par) {
  return par[0] * std::exp(-0.5 / (1 - std::pow(par[5], 2)) *
...
void makeResolutionPlots(std::vector<std::string> paths) {
  ...
  if (hist_data_vec.size() > 1) {
    ...
    /*  NeatPlotting::TextStyle text_style;
     ...
     NeatPlotting::PlotLabel label2("0.097 + 0.825 exp(-0.476 p_{lab})",
     text_style); label1.setRelativePosition(0.45, 0.95);
     ...
     NeatPlotting::PlotLabel label4("0.093 + 0.531 exp(-0.540 p_{lab})",
     text_style); label3.setRelativePosition(0.45, 0.83);
