...
scriptpath=$PWD
macropath="${VMCWORKDIR}/macro/lmd"
cd $macropath
...
if [ 0 -eq "$?" ] || [ 2 -eq "${force_level}" ]; then
  ...
  echo "starting up a pandaroot simulation..."
   ...
   # root -l -b -q 'runKoaPixel0SimBox.C('${num_evts}','${start_evt}',"'${workpathname}'",'$verbositylvl',2112,'${mom}','${numTrks}','${random_seed}')' > /dev/null 2>&1
  #else
    root -l -b -q 'runKoaPixel0SimDPM.C('${num_evts}','${start_evt}','${mom}',"'${gen_filepath}'", "'${workpathname}'",'$beamX0', '$beamY0', '${targetZ0}', '${beam_widthX}', '${beam_widthY}', '${target_widthZ}', '${beam_gradX}', '${beam_gradY}', '${beam_grad_sigmaX}', '${beam_grad_sigmaY}', "'${lmd_geometry_filename}'", "'${misalignment_matrices_path}'", '${use_point_transform_misalignment}', '$verbositylvl')'
  #fi
  if [ "${debug}" -eq 0 ]; then 
    cp $workpathname/Koala_MC_${start_evt}.root ${path_mc_data}/Koala_MC_${start_evt}.root
    cp $workpathname/Koala_Params_${start_evt}.root ${path_mc_data}/Koala_Params_${start_evt}.root
  fi
else
  if [ "${debug}" -eq 0 ]; then
    cp ${path_mc_data}/Koala_MC_${start_evt}.root $workpathname/Koala_MC_${start_evt}.root
    cp ${path_mc_data}/Koala_Params_${start_evt}.root $workpathname/Koala_Params_${start_evt}.root
  fi
fi
