...
job = himster.Job(resource_request, './runBoxGen.sh', 'runBoxGen_' + dirname_cleaned, basedir + '/' + dirname + '/runBoxGen_' + dirname_cleaned + '-%a.log')
...
job.add_exported_user_variable('theta_min', str(args.theta_min))
job.add_exported_user_variable('theta_max', str(args.theta_max))
...
job.add_exported_user_variable('use_recoil_mom', boolval)
