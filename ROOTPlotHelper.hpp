...
// ==================== namespace helper structures & functions ====================
  ...
  inline void setDiffColorPalette(
  ...
  inline void setNormalColorPalette(const unsigned int color_gradient_steps =
      255) {
    const Int_t NRGBs = 5;
    ...
    if (normal_colors == 0) {
      ...
      Int_t FI = TColor::CreateGradientColorTable(NRGBs, Length, Red, Green,
          Blue, color_gradient_steps);
      for (unsigned int i = 0; i < color_gradient_steps; i++)
  ...
  template<class T> class DrawableDataObjectStylePair {
  ...
  public:
    ...
    DrawableDataObjectStylePair(T data_object_, std::string draw_option_) :
...
// data structures and wrappers
  ...
  public:
    ...
    void initAxis(const PadBoundaries &pad_boundaries) {
      ...
      if (pad_boundaries.is_2d) {
        ...
        axis_dummy->GetZaxis()->SetRangeUser(z_axis_range.low,
            z_axis_range.high);
  ...
  class PlotBundle {
    std::vector<DrawableDataObjectStylePair<TH1*> > histograms;
    std::vector<DrawableDataObjectStylePair<TGraph*> > graphs;
    std::vector<DrawableDataObjectStylePair<TGraph2D*> > graphs2d;

    bool drawHistsAndGraphs(const PlotStyle &plot_style) const {
      if (hasDrawables()) {
        std::vector<DrawableDataObjectStylePair<TH1*> >::const_iterator hist_it;
        ...
        std::vector<DrawableDataObjectStylePair<TGraph*> >::const_iterator graph_it;
        ...
        std::vector<DrawableDataObjectStylePair<TGraph2D*> >::const_iterator graph2d_it;
  ...
  public:
    ...
    std::vector<DrawableDataObjectStylePair<TH1*> > getHistograms() const {
      return histograms;
    }
    std::vector<DrawableDataObjectStylePair<TGraph*> > getGraphs() const {
      return graphs;
    }
    std::vector<DrawableDataObjectStylePair<TGraph2D*> > getGraphs2d() const {
      return graphs2d;
    }
    ...
    void addHistogram(TH1* hist, const DataObjectStyle &style) {
      ...
      histograms.push_back(
          DrawableDataObjectStylePair<TH1*>(hist, style.draw_option));
    ...
    void addGraph(TGraph* graph, const DataObjectStyle &style) {
      ...
      graphs.push_back(
          DrawableDataObjectStylePair<TGraph*>(graph, style.draw_option));
    }
    void addGraph(TGraph2D* graph, const DataObjectStyle &style) {
      applyMarkerStyleToDrawableObject(graph, style.marker_style);
      applyLineStyleToDrawableObject(graph, style.line_style);
      graphs2d.push_back(
          DrawableDataObjectStylePair<TGraph2D*>(graph, style.draw_option));
    }
    ...
    void drawPlotAccessories() {
      ...
      // draw parallel lines
      for (unsigned int i = 0; i < plot_decoration.x_parallel_lines.size();
          i++) {
      ...
      for (unsigned int i = 0; i < plot_decoration.y_parallel_lines.size();
          i++) {
...
// ==================== graph helper functions ====================
    ...
    TGraphAsymmErrors* makeDifferenceGraph(const TGraphAsymmErrors *g1,
      ...
      if (g1->GetN() != g2->GetN())
        std::cout
            << "Warning: the two graphs have a different number of events!"
            << std::endl;
    ...
    TGraphAsymmErrors* makeResidualZeroGraph(
        const TGraphAsymmErrors *g1) const {
      std::vector<GraphPoint> points;
      points.reserve(g1->GetN());
...
// to speed things up reserve memory
    ...
    TGraphAsymmErrors* makeRatioGraph(const TGraphAsymmErrors *g1,
      ...
      if (g1->GetN() != g2->GetN())
        std::cout
            << "Warning: the two graphs have a different number of points!"
            << std::endl;
...
// ==================== histogram helper functions ====================
    ...
    TH1D* makePullHistogram(TH2D *h1, TH2D *h2) const {
      TH1D* reldiff;
      if (h1 && h2) {
        reldiff = new TH1D("", "", 400, -2, 2);
        for (unsigned int ix = 1; ix < h1->GetXaxis()->GetNbins() + 1; ++ix) {
          for (unsigned int iy = 1; iy < h1->GetYaxis()->GetNbins() + 1; ++iy) {
            int bin = h1->GetBin(ix, iy);
            reldiff->Fill((h1->GetBinContent(bin)-h2->GetBinContent(bin))/h1->GetBinError(bin));
          }
        }
      }
      return reldiff;
    ...
    TH2D* rescaleAxis(TH2D* hist, double scale_factor_x,
  ...
  private:
    ...
    GausfitPars fitGauss(TH1D *hist) const {
      ...
      gp.sigma = std::make_pair(gausfit.GetParameter(2),
          gausfit.GetParError(2));
      return gp;
    }

    ValueErrorPair determineMeanAndRMS(
    ...
    std::map<double, TH1D*> createDependencyHistogramMap() const {
      ...
      for (dependency_values = dependency_values_map.begin();
          dependency_values != dependency_values_map.end();
          dependency_values++) {
        ...
        dependency_hist_map[dependency_values->first] = new TH1D("dist", "",
            100, hist_bounds.first - sigma_allowance * hist_bounds.second,
