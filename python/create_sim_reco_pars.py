...
def genOffsetIPconfigs() -> None:
    Path("./expConfigs/PANDA/IPoffset").mkdir(parents=True, exist_ok=True)
    ...
    for momentum in [1.5, 15]:
        ...
        for offset in [-1.0, -0.5, -0.25, 0, 0.25, 0.5, 1.0]:
            ...
            write_params_to_file(
                experiment,
                Path("."),
                f"expConfigs/PANDA/IPoffset/{momentum}:{offset}.config",
                overwrite=True,
            )
...
parser.add_argument(
    "-ip",
    dest="ipOffset",
    help="generate configs with offset IP positions",
