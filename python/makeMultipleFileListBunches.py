...
def makeFileListBunches(directory):
    ...
    while len(good_files) > 0:
        ...
        createFileListFile(output_bunch_dir + '/filelist_' +
                           str(file_list_index) + '.txt', chunk_of_good_files)
        file_list_index += 1


parser = argparse.ArgumentParser(
    description='Script for going through whole directory trees and generating filelist bunches for faster lmd data creation.', formatter_class=argparse.RawTextHelpFormatter)
...
parser.add_argument('--files_per_bunch', metavar='files_per_bunch',
                    type=int, default=4, help='number of root files used for a bunch')
parser.add_argument('--maximum_number_of_files', metavar='maximum_number_of_files',
                    type=int, default=-1, help='total number of root files to use')
parser.add_argument('--directory_pattern', metavar='directory_pattern', type=str,
                    default='.*', help='Only directories with according to this pattern will be used.')
