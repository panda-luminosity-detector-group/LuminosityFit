...
mydouble BoxModel2D::eval(const mydouble *x) const {
  if (x[0] >= lower_edge_var1->getValue() &&
      x[0] <= upper_edge_var1->getValue()) {
    if (x[1] >= lower_edge_var2->getValue() &&
        x[1] <= upper_edge_var2->getValue()) {
...
void BoxModel2D::initModelParameters() {
  ...
  lower_edge_var1 = getModelParameterSet().addModelParameter("lower_edge_var1");
  upper_edge_var1 = getModelParameterSet().addModelParameter("upper_edge_var1");
  lower_edge_var2 = getModelParameterSet().addModelParameter("lower_edge_var2");
  upper_edge_var2 = getModelParameterSet().addModelParameter("upper_edge_var2");
}
