...
protected:
  ...
  // define these here
  mydouble multiply(std::shared_ptr<Model> m1, std::shared_ptr<Model> m2,
                    const mydouble *x) const;

  mydouble add(std::shared_ptr<Model> m1, std::shared_ptr<Model> m2,
               const mydouble *x) const;
...
public:
  ...
  virtual mydouble
  Integral(const std::vector<DataStructs::DimensionRange> &ranges,
