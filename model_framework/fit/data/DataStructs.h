...
struct binned_data_point {
  ...
  mydouble z_error;
  mydouble scale;
  binned_data_point() : z(0.0), z_error(0.0), scale(1.0) {}
};
...
struct DimensionRange {
  ...
  DimensionRange() : range_low(0.0), range_high(0.0), is_active(false) {}

  DimensionRange(mydouble low, mydouble high)
      : range_low(low), range_high(high), is_active(true) {}

  mydouble getDimensionLength() const { return (range_high - range_low); }

  mydouble getDimensionMean() const { return (range_high + range_low) / 2.0; }
