...
SmearingConvolutionModel2D::SmearingConvolutionModel2D(
    std::string name_, std::shared_ptr<Model2D> first_,
    std::shared_ptr<Model2D> second_)
    : Model2D(name_), first(first_), second(second_), divisions_var1(100),
      divisions_var2(100) {
...
mydouble SmearingConvolutionModel2D::eval(const mydouble *x) const {
  ...
  // get domain of the second model at the evaluation point x and only scan in
  // surrounding
  second->evaluate(x);
  unsigned int i1min =
      (unsigned int)(0.9 *
                     (x[0] + second->getVar1DomainLowerBound() -
                      first->getVar1DomainLowerBound()) /
                     division_width_var1);
  unsigned int i1max =
      (unsigned int)(1.1 *
                     (x[0] + second->getVar1DomainLowerBound() +
  ...
  // now loop over these subinvervals and integrate the to be smeared model in
  // this subinterval region and integrate the smearing model in the shifted
  // region to the evaluation point
  for (unsigned int i1 = i1min; i1 < i1max; i1++) {
    for (unsigned int i2 = i2min; i2 < i2max; i2++) {
      ...
      temp_range_first[0].range_high =
          temp_range_first[0].range_low + division_width_var1;
      temp_range_first[1].range_low = range_low_var2 + division_width_var2 * i2;
      temp_range_first[1].range_high =
          temp_range_first[1].range_low + division_width_var2;
      ...
      /*std::cout << second->getVar1DomainLowerBound() << "-"
       << second->getVar1DomainLowerBound() + second->getVar1DomainRange() <<
       std::endl; std::cout << second->getVar2DomainLowerBound() << "-"
       << second->getVar2DomainLowerBound() + second->getVar2DomainRange() <<
       std::endl; std::cout<<"--------------"<<std::endl; std::cout << "theta,
       phi= " << x[0] << ", " << x[1] << std::endl;
       std::cout<<"--------------"<<std::endl;
      ...
      // std::cout<<x[0]<< " "<<interval_low<<"
      // "<<smear_interval_low<<std::endl;
