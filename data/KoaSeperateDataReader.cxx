...
void KoaSeperateDataReader::initDataStream() {
  ...
  geane_tree.SetBranchAddress(
      "LMDCompa", &geane_tracks); // Tracks with parabolic parametrisation
  geane_tree.SetBranchStatus("LMDCompa*",
...
KoaSeperateDataReader::getEntry(unsigned int i) {
  if (mc_entries > 0)
    MC_tree.GetEntry(i);
  ...
  for (int ik = 0; ik < true_points->GetEntriesFast(); ik++) {
    ...
    if (-2212 == ((KoaMCTrack *)true_tracks->At(lmd_point->GetTrackID()))
                     ->GetPdgCode()) {
  ...
  for (Int_t iN = 0; iN < lmd_tracks->GetEntriesFast(); iN++) {
    ...
    /*	double HitX = trackpar.GetX();
            ...
            double a = (PX * HitX + PY * HitY + PZ * HitZ)/(PX * PX + PY * PY +
    PZ * PZ); double Xo = HitX - (PX * a); double Yo = HitY - (PY * a); double
    Zo = HitZ - (PZ * a);
  ...
  for (Int_t iN = 0; iN < geane_tracks->GetEntries(); iN++) {
    ...
    double a =
        (PX * HitX + PY * HitY + PZ * HitZ) / (PX * PX + PY * PY + PZ * PZ);
    double Xo = HitX - (PX * a);
    double Yo = HitY - (PY * a);
    double Zo = HitZ - (PZ * a);
