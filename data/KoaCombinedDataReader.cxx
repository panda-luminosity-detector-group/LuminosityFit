...
KoaCombinedDataReader::getEntry(unsigned int i) {
  ...
  for (Int_t iN = 0; iN < comp_array->GetEntries(); iN++) {
    ...
    for (unsigned int tracki = 0; tracki < koa_array->GetEntries(); tracki++) {
      ...
      double koathet =
          1.559 - lmdthet; // angle between fowards and backwards particle is a
                           // little bit smaller than ninety°
      ...
      double IPp1 = HitZ - lz; // This are all terms for the calculation of a
                               // cut between two lines in two dimensions
