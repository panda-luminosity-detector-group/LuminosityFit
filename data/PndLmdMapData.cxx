...
PndLmdMapData::PndLmdMapData() : entries_per_file(1000000) {}
PndLmdMapData::PndLmdMapData(const PndLmdMapData &lmd_hist_data_)
    : PndLmdAbstractData(lmd_hist_data_), hit_map_2d(lmd_hist_data_.hit_map_2d),
      entries_per_file(lmd_hist_data_.entries_per_file),
      data_tree_names(lmd_hist_data_.data_tree_names),
      data_tree_file_url(lmd_hist_data_.data_tree_file_url) {
...
void PndLmdMapData::addData(const std::vector<double> &values) {
  int mc_idx((values[2] - primary_dimension.dimension_range.getRangeLow()) /
             primary_dimension.bin_size);
  int mc_idy((values[3] - secondary_dimension.dimension_range.getRangeLow()) /
             secondary_dimension.bin_size);
  Point2D mc_point(primary_dimension.dimension_range.getRangeLow() +
  ...
  ++(mc_bin.total_count);
  int rec_idx((values[0] - primary_dimension.dimension_range.getRangeLow()) /
              primary_dimension.bin_size);
  int rec_idy((values[1] - secondary_dimension.dimension_range.getRangeLow()) /
              secondary_dimension.bin_size);
  Point2D rec_point(primary_dimension.dimension_range.getRangeLow() +
...
void PndLmdMapData::readFromRootTrees() {
  ...
  for (auto const &tree_name : data_tree_names) {
    ...
    auto result = std::find_if(
        branch_names.begin(), branch_names.end(), [](const std::string &s) {
          return s.find("reco_points_x") != std::string::npos;
        });
